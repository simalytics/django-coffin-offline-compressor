...
class Command(NoArgsCommand):
    ...
    def compress(self, log=None, **options):
        ...
        for loader in self.get_loaders():
            try:
                ...
                # Ignore site-packages for now
                paths = set([p for p in paths if 'site-packages' not in p])
    ...
    def get_nodelist(self, node):
        ...
        if hasattr(node, "else_"):
            results = getattr(node, "body", getattr(node, "nodes", [])) + getattr(node, "else_", [])
        else:        
            results = getattr(node, "body", getattr(node, "nodes", []))
        return results
